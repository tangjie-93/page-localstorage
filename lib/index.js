    ...
    LocalStorageClass.prototype = {
        ...
        */
        setStorage (...args) {
            ...
            //为页面设置keys数组
            if (this.pageName) {
                this._setItem(`${this.pageName}:${PAGE}`, this.storageKeys);
                //避免页面刷新获取不到页面keys的问题
                localStorage.setItem(`${PAGENAMES}:${PAGENAMES}`, LocalStorageClass.pageNames);
            }
        },
        /**
         * 按需删除页面的缓存
        ...
        removeStorageByPageName (pageName = this.pageName) {
            ...
            if (index > -1) {
                ...
                pageNames.length ? localStorage.setItem(`${PAGENAMES}:${PAGENAMES}`, pageNames) : localStorage.removeItem(`${PAGENAMES}:${PAGENAMES}`)
            }
        },
        //删除所有页面的缓存
